...
def maps(name,  m , x):
    ...
    if name == 'tent':
        if x < 0.5:
            # in this definition m varies [0,2]
            return m* x
        if x > 0.5:
            return m * (1.0 - x)
    if name =='ecology':
...
def plot_bifurcation(name, m_min, m_max, step):
    ...
    for m in my_range(m_min, m_max , step):
        for i in range (1,2001,1):   #skip transients
            x = maps(map_name , m , x)
...
# plot_bifurcation('logistic', 2.0 , 4.0 , .001)
# plot_bifurcation('tent',1.0 , 2.0 , .001)
# plot_bifurcation('gaussian', -1.0, 1.0, .001)
# plot_bifurcation('tent_v2', 0.0, 1.0, 0.01)
